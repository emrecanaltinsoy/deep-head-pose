...
def parse_args():
    ...
    parser.add_argument(
        "--gpu",
        dest="gpu_id",
        help="GPU device id to use, if not set use CPU",
        default=None,
        type=int,
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    reg_criterion = nn.MSELoss().to(device=device)
    # Regression loss coefficient
    alpha = args.alpha

    softmax = nn.Softmax().to(device=device)
    idx_tensor = [idx for idx in range(66)]
    idx_tensor = Variable(torch.FloatTensor(idx_tensor)).to(device=device)
    ...
    for epoch in range(num_epochs):
        for i, (images, labels, cont_labels, name) in enumerate(train_loader):
            ...
            # Binned labels
            label_yaw = Variable(labels[:, 0]).to(device=device)
            label_pitch = Variable(labels[:, 1]).to(device=device)
            label_roll = Variable(labels[:, 2]).to(device=device)

            # Continuous labels
            label_yaw_cont = Variable(cont_labels[:, 0]).to(device=device)
            label_pitch_cont = Variable(cont_labels[:, 1]).to(device=device)
            label_roll_cont = Variable(cont_labels[:, 2]).to(device=device)
            ...
            grad_seq = [
                torch.ones(1)[0].to(device=device) for _ in range(len(loss_seq))
            ]
            optimizer.zero_grad()
